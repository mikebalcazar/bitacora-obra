 ...
 * ya se mandaron siguen sirviendo y todos acaban en el dominio. Sólo la
 * PANTALLA (lecturas GET/HEAD de la página y sus archivos): `/api/*`,
 * `/files/*` y `/descargas/*` NO se redirigen, porque las apps empacadas de
 * Android y Windows ya instaladas le pegan a workers.dev con su token y un
...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      const ida = aDominioPropio(req, env, url);
      if (ida) return ida;
      let r;
