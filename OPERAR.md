...
curl -s -o /dev/null -w "%{http_code}\n" -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/bitacora-obra
```
...
```bash
git clone "https://x-access-token:${T}@github.com/mikebalcazar/bitacora-obra.git" /home/claude/repo
...
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/bitacora-obra/actions/runs?per_page=3" \
...
curl -s -X POST -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/bitacora-obra/pulls \
  -d '{"title":"…","head":"claude/…","base":"main","body":"…"}'
# y con el número que devuelve:
curl -s -X PUT -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/bitacora-obra/pulls/<N>/merge \
...
```bash
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/bitacora-obra/actions/permissions/workflow"
```
...
```bash
curl -s -X POST -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/bitacora-obra/actions/workflows/verificar.yml/dispatches" \
